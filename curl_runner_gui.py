#!/usr/bin/env python3
"""
curl_runner_gui.py — Curl Runner Desktop (v2)
Tính năng: History, Collections, Environment & Variables
...
# ══════════════════════════════════════════════
# DATA STORE  (lưu tại ~/.curl_runner/)
# ══════════════════════════════════════════════
...
class CurlRunnerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(topbar, text="v2  ·  History · Collections · Environments",
        ...
        outer.add(self._build_center(outer),  minsize=340, width=520)
    ...
    def _build_history_panel(self, parent):
        ...
        self.hist_search = tk.Entry(sf, bg=BG3, fg=TEXT_DIM, insertbackground=ACCENT,
    ...
    def _hist_focus(self, on):
        ...
        if on and self.hist_search.get() == ph:
            ...
            self.hist_search.config(fg=TEXT)
        elif not on and not self.hist_search.get():
            self.hist_search.insert(0, ph)
            self.hist_search.config(fg=TEXT_DIM)
    ...
    def _refresh_history_list(self, *_):
        ...
        for item in reversed(self.history[-300:]):
            ...
            line   = f"{method} {url}"
            if q and q not in line.lower() and q not in sc: continue
    ...
    def _load_from_history(self, _=None):
        ...
        if not sel: return
        # Tính index trong mảng history (đã reverse)
        visible = []
        q = self.hist_search.get().lower()
        if q == "🔍 tìm kiếm...": q = ""
        for item in reversed(self.history[-300:]):
        ...
        if 0 <= idx < len(visible):
            self._set_curl(visible[idx].get("curl",""))

    def _history_ctx(self, event):
        idx_lb = self.hist_list.nearest(event.y)
        self.hist_list.selection_clear(0, "end")
        self.hist_list.selection_set(idx_lb)
        m = tk.Menu(self, tearoff=0, bg=BG3, fg=TEXT, activebackground=ACCENT)
        m.add_command(label="▶  Load vào editor", command=self._load_from_history)
        m.add_command(label="➕ Lưu vào Collection",
                      command=lambda: self._save_to_coll_dialog(
                          self._get_visible_history()[idx_lb].get("curl","")
                          if idx_lb < len(self._get_visible_history()) else ""))
        m.add_separator()
        m.add_command(label="🗑 Xóa mục này",
                      command=lambda: self._del_hist_item(idx_lb))
    ...
    def _del_hist_item(self, lb_idx):
        visible = self._get_visible_history()
        if lb_idx >= len(visible): return
        target_id = visible[lb_idx].get("id")
        self.history = [h for h in self.history if h.get("id") != target_id]
        ...
        for col_name, items in self.collections.items():
            ...
            for it in items:
                ...
                self.coll_tree.insert(node, "end",
                                      text=f"  {method:5} {it.get('name','')}",
                                      values=(col_name, it["id"]),
                                      tags=("item",))
    ...
    def _coll_ctx(self, event):
        ...
        if vals and len(vals) >= 2:
            col_name, item_id = vals[0], vals[1]
            m.add_command(label="▶  Load vào editor", command=self._load_from_collection)
            m.add_command(label="✏  Đổi tên",
                          command=lambda: self._rename_coll_item(col_name, item_id))
            m.add_separator()
            m.add_command(label="🗑 Xóa request",
    ...
    def _save_to_coll_dialog(self, curl_str):
        ...
        if not self.collections:
            if messagebox.askyesno("Chưa có Collection", "Tạo collection mới?"):
        ...
        tk.Label(win, text="Collection:", font=self.fn_label, bg=BG, fg=TEXT_DIM).pack(pady=(16,2))
        col_var = tk.StringVar(value=list(self.collections.keys())[0])
        ttk.Combobox(win, textvariable=col_var, values=list(self.collections.keys()),
                     state="readonly", font=self.fn_label).pack(padx=20, fill="x")

        tk.Label(win, text="Tên request:", font=self.fn_label, bg=BG, fg=TEXT_DIM).pack(pady=(10,2))
        name_e = tk.Entry(win, font=self.fn_label, bg=BG3, fg=TEXT,
                          insertbackground=ACCENT, relief="flat")
        name_e.pack(padx=20, fill="x")
        name_e.insert(0, "New Request")
    ...
    def _del_collection(self, col_name):
        if messagebox.askyesno("Xác nhận", f"Xóa collection '{col_name}'?"):
    ...
    def _open_env_editor(self):
        ...
        def del_env():
            ...
            if len(self.environments) <= 1:
                messagebox.showinfo("","Phải giữ ít nhất 1 environment."); return
            if messagebox.askyesno("Xác nhận", f"Xóa '{n}'?", parent=win):
                self.environments.pop(n, None)
                env_cb["values"] = list(self.environments.keys())
                env_sel.set(list(self.environments.keys())[0])
                load_vars()
        ...
        tree.column("variable", width=200)
        ...
        def add_row():
            tree.insert("", "end", values=("NEW_VAR","value"))
        ...
        def save_close():
            ...
            for iid in tree.get_children():
                ...
                if k.strip():
                    self.environments[env_name][k.strip()] = v
        ...
        self._mkbtn(bf, "＋ Thêm dòng", add_row, side="left")
        self._mkbtn(bf, "🗑 Xóa dòng",  del_row, side="left", pad=(6,0))
    ...
    def _build_center(self, parent):
        ...
        self.env_hint = tk.Label(hdr, text="", font=self.fn_small, bg=BG, fg=TEXT_DIM)
        self.env_hint.pack(side="right")

        tb = tk.Frame(frame, bg=BG)
        tb.pack(fill="x", pady=(0,4))
        self._mkbtn(tb, "📂 Import File",        self._import_file,            side="left")
        self._mkbtn(tb, "✂ Xóa",                self._clear_input,            side="left", pad=(6,0))
        self._mkbtn(tb, "➕ Lưu vào Collection", self._save_current_to_coll,   side="left", pad=(6,0))
        ...
        sb = tk.Scrollbar(wrap, command=self.curl_text.yview,
                          bg=BG3, troughcolor=BG2, bd=0)
        self.curl_text.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        self.curl_text.pack(fill="both", expand=True, padx=1, pady=1)
        self._set_placeholder()
        self.curl_text.bind("<FocusIn>",    self._clear_ph)
        self.curl_text.bind("<FocusOut>",   self._restore_ph)
        self.curl_text.bind("<KeyRelease>", self._update_env_hint)
        ...
        self.var_decode   = tk.BooleanVar(value=True)
        self._chk(opt, "Verify SSL",      self.var_ssl)
        self._chk(opt, "Follow Redirect", self.var_redirect)
        self._chk(opt, "Auto Decode",     self.var_decode)
        tk.Label(opt, text="Timeout (s):", font=self.fn_label,
                 bg=BG, fg=TEXT_DIM).pack(side="left", padx=(10,4))
        self.timeout_var = tk.StringVar(value="30")
        tk.Entry(opt, textvariable=self.timeout_var, font=self.fn_monos,
        ...
        self.send_btn.pack(fill="x", pady=(10,2))
        self.left_status = tk.Label(frame, text="", font=self.fn_label,
                                    bg=BG, fg=TEXT_DIM, anchor="w")
        self.left_status.pack(fill="x")
        return frame

    def _update_env_hint(self, _=None):
        text  = self.curl_text.get("1.0","end")
        found = re.findall(r'\{\{(\w+)\}\}', text)
        if not found:
            self.env_hint.config(text=""); return
        ...
        self.env_hint.config(text="  ".join(parts),
                             fg=RED_C if bad else GREEN)
    ...
    def _build_right(self, parent):
        ...
        self.status_badge = tk.Label(ss, text="—", font=self.fn_stat,
                                     bg=BG, fg=TEXT_DIM)
        ...
        tr.pack(fill="x")
        for lbl, val in [("Body","body"),("Headers","headers"),("Request Info","info")]:
            btn = tk.Button(tr, text=lbl, font=self.fn_label,
                            ...
                            command=lambda v=val: self._show_tab(v), bd=0)
            btn.pack(side="left", padx=(0,2))
            setattr(self, f"tab_{val}", btn)
    ...
    def _show_tab(self, val):
        for v in ("body","headers","info"):
            getattr(self, f"{v}_frame").pack_forget()
            getattr(self, f"tab_{v}").config(
    ...
    # ── Placeholder ───────────────────────────
    PH = ("Paste hoặc nhập curl command vào đây...\n\n"
    ...
    def _set_placeholder(self):
        self.curl_text.insert("1.0", self.PH)
        self.curl_text.config(fg="#555b70")
        self._ph = True

    def _clear_ph(self, _=None):
        if getattr(self, "_ph", False):
            self.curl_text.delete("1.0","end")
            self.curl_text.config(fg=TEXT)
            self._ph = False

    def _restore_ph(self, _=None):
        if not self.curl_text.get("1.0","end").strip():
            self._set_placeholder()

    def _set_curl(self, curl_str):
        self._clear_ph()
        self.curl_text.delete("1.0","end")
        self.curl_text.insert("1.0", curl_str)
        self._update_env_hint()
    ...
    def _import_file(self):
        ...
        try:
            ...
            self.left_status.config(text=f"📂 Đã import: {Path(path).name}", fg=GREEN)
    ...
    def _clear_input(self):
        self.curl_text.delete("1.0","end")
        self._restore_ph()
        self.left_status.config(text="")
        self.env_hint.config(text="")

    def _save_current_to_coll(self):
        curl_str = self.curl_text.get("1.0","end").strip()
        if not curl_str or getattr(self,"_ph",False):
    ...
    def _send(self):
        ...
        self.send_btn.config(state="disabled", text="⏳  Đang gửi...")
        self.left_status.config(text="Đang gửi request...", fg=TEXT_DIM)
        ...
        def worker():
            try:
                ...
                parsed["verify_ssl"]      = self.var_ssl.get()
                parsed["allow_redirects"] = self.var_redirect.get()
                try:    parsed["timeout"] = float(self.timeout_var.get())
                except: parsed["timeout"] = 30
                resp, elapsed = execute_request(parsed)
                self.after(0, lambda: self._display(parsed, resp, elapsed, curl_str))
            except Exception as e:
                self.after(0, lambda: self._show_error(str(e)))

        threading.Thread(target=worker, daemon=True).start()

    def _display(self, parsed, resp, elapsed, original_curl):
        ...
        if not self.var_decode.get():
            # Chế độ RAW — không decode, trả nguyên bytes dưới dạng hex/latin-1
            body_text = resp.content.decode("latin-1")
            self._detected_charset = "raw (no decode)"
        elif charset_match:
            # Server khai báo charset rõ ràng → dùng luôn
            charset = charset_match.group(1).strip()
            self._detected_charset = f"{charset}  (từ Content-Type header)"
            try:
                body_text = resp.content.decode(charset, errors="replace")
            except (LookupError, UnicodeDecodeError):
                body_text = resp.content.decode("utf-8", errors="replace")
                self._detected_charset += " → fallback utf-8"
        elif HAS_DETECT and resp.content:
            # Không có charset → auto-detect từ raw bytes
            result  = detect_encoding(resp.content)
            charset = result.best().encoding if hasattr(result, "best") else "utf-8"
            self._detected_charset = f"{charset}  (auto-detected)"
            try:
                body_text = resp.content.decode(charset or "utf-8", errors="replace")
            except (LookupError, UnicodeDecodeError):
                body_text = resp.content.decode("utf-8", errors="replace")
                self._detected_charset = "utf-8  (fallback)"
        else:
            body_text = resp.content.decode("utf-8", errors="replace")
            self._detected_charset = "utf-8  (fallback — charset-normalizer not installed)"

        self._write_body(body_text, resp.headers.get("Content-Type",""))
        self._write_headers(resp.headers)
        self._write_info(parsed, resp, elapsed)

        self.send_btn.config(state="normal", text="▶  SEND REQUEST")
        url_s = parsed['url'][:55] + ("..." if len(parsed['url'])>55 else "")
        self.left_status.config(text=f"✓  {parsed['method']}  →  {url_s}", fg=GREEN)
    ...
    def _insert_json(self, w, text):
        """
        Token-by-token JSON syntax highlighter.
        Handles all values on every line correctly — no more missing values.
        """
        TOKEN_RE = re.compile(
            r'("(?:[^"\\]|\\.)*")'          # string  — group 1
            r'|(-?\d+(?:\.\d+)?(?:[eE][+\-]?\d+)?)'  # number — group 2
            r'|(true|false|null)'            # keyword — group 3
            r'|([{}\[\]:,])'                 # punct   — group 4
            r'|(\s+)'                        # space   — group 5
        )
        pos        = 0
        expect_key = True  # True right after { or ,  →  next string = key

        for m in TOKEN_RE.finditer(text):
            if m.start() > pos:
                w.insert("end", text[pos:m.start()], "plain")
            pos = m.end()

            s_val, n_val, kw_val, punct, space = m.groups()

            if space is not None:
                w.insert("end", space, "plain")
            elif punct is not None:
                w.insert("end", punct, "plain")
                if punct in ('{', ','):
                    expect_key = True
                elif punct == ':':
                    expect_key = False
                ...
                if expect_key:
                    w.insert("end", s_val, "key")
                    expect_key = False
                else:
                    w.insert("end", s_val, "str")
            elif n_val is not None:
                w.insert("end", n_val, "num")
            elif kw_val is not None:
                w.insert("end", kw_val, "bool")

        if pos < len(text):
            w.insert("end", text[pos:], "plain")
    ...
    def _write_info(self, parsed, resp, elapsed):
        w = self.info_tw
        w.config(state="normal"); w.delete("1.0","end")
        ...
        cs_m  = __import__("re").search(r'charset=([^\s;]+)', ct_h, __import__("re").I)
        if cs_m:
            enc_display = f"{cs_m.group(1).strip()}  (từ Content-Type header)"
        else:
            enc_display = getattr(self, "_detected_charset", "utf-8 (fallback)")
            enc_display += "  (auto-detected)"
        row("Encoding",  enc_display)
        w.config(state="disabled")

    def _show_error(self, msg):
        self.send_btn.config(state="normal", text="▶  SEND REQUEST")
        self.status_badge.config(text="ERROR", fg=RED_C)
        self.left_status.config(text=f"❌ {msg[:80]}", fg=RED_C)
    ...
    def _copy_response(self):
        try:
            ...
            if c:
                ...
                self.left_status.config(text="✓ Đã copy response body", fg=GREEN)
        except: pass
    ...
    def _save_response(self):
        ...
        if path:
            with open(path,"wb") as f: f.write(self._last_response.content)
            self.left_status.config(text=f"✓ Đã lưu → {Path(path).name}", fg=GREEN)
